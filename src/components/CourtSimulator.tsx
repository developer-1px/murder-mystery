...
import type { Card, Character, MemoryStagesDocument, NpcGroupsDocument, Scenario } from '../domain/types'
...
export function CourtSimulator({ scenario, npcGroups, memoryStages, inspectionCards }: { scenario: Scenario; npcGroups: NpcGroupsDocument; memoryStages: MemoryStagesDocument; inspectionCards: Card[] }) {
    ...
    <div className="court-simulator__rounds">
      {result.rounds.slice(0, court).map((entries, roundIndex) => <section key={roundIndex} className={roundIndex + 1 === court ? 'is-current' : ''}>
        ...
        <div className="court-simulator__cards">{entries.map(entry => <article key={`${entry.source}-${entry.actor.id}-${entry.card.id}`}>
          <p>{entry.source === 'inspection' ? <><strong>로웬</strong> · 검사 결과</> : <><strong>{entry.actor.name}</strong> · 제출{entry.target && <> → {entry.target.name}</>}</>}</p>

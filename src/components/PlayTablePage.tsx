import { ActionLink, Disclosure, DisclosureSummary, Button } from '../design-system/controls'
...
function phaseTitle(session: PlaySession) {
  ...
  return ({ inspection: '로웬의 검시', rumor: '궁정의 소문', testimony: '사람의 흔적', investigation: '장소에 남은 증거', discussion: '문이 닫힌 사이', truth_exchange: '진실을 맡기는 시간', truth_choice: '밝힐 진실과 묻을 진실', accusation: '당신이 지목한 범인', defense: '마지막 변론', indictment: '로웬의 최종 기소', complete: '진실 공개 결과' } as Record<string, string>)[session.phase]
}
...
export function PlayTablePage() {
  ...
  if (stored.error) return <section className="play-recovery">
    ...
    <p>{stored.error}</p>
    <Button variant="primary" className="play-primary" disabled={recovering} onClick={async () => {
      setRecovering(true)
      setRecoveryError('')
      try {
    ...
    <ActionLink size="compact" to="/guide">게임 진행 읽기</ActionLink>
  </section>
  ...
  return <>
    {warning && <div className="play-warning ui-notice" data-tone="warning" role="status"><span>{warning}</span><Button onClick={() => setWarning(undefined)} aria-label="안내 닫기"><Icon name="close" /></Button></div>}
    ...
    {params.get('history') === '1' && <div className="play-history ui-panel" data-elevation="floating">
      <header><h3>플레이 기록</h3><Button size="compact" variant="ghost" onClick={() => update({ history: null })}>닫기 <Icon name="close" /></Button></header>
      <p>과거에서 다른 선택을 하면 원래 진행을 남기고 분기합니다. 기록은 이 브라우저에 저장됩니다.</p>
      {history.branches.map(item => <Disclosure key={item.id} open={item.id === branch.id}>
        <DisclosureSummary>{item.name} · {item.snapshots.length - 1}회 진행</DisclosureSummary>
        {item.snapshots.map((snapshot, index) => <ActionLink size="compact" key={index} onClick={() => setRunAt(null)} aria-current={item.id === branch.id && index === cursor ? 'step' : undefined} to={playPath(item.id, index)}>
          <b>{String(index).padStart(2, '0')}</b>{snapshot.log.at(-1)?.text ?? '게임 준비'}
        </ActionLink>)}
      </Disclosure>)}
      <Button variant="primary" className="play-primary" onClick={newGame}>새 플레이 시작</Button>
      <ActionLink size="compact" to="/table/archive">이전 카드 배치 기록 열기</ActionLink>
    </div>}
  </>
}
...
function PlayTable({ session, act, newGame, cursor, lastStep, onUndo, onRedo, autoRunning, onToggleAuto }: {
  ...
  const policy: CardTablePolicy = {
    ...
    choice: ownTurn && session.choice ? { sourceId: session.choice.deckId, cards: session.choice.cardIds.map(cardId => ({ cardId, faceUp: true })), onPick: take,
      note: `${actor.name} · 1장을 선택하거나 손패로 끌어오세요. 나머지는 덱 아래에 넣습니다.` } : undefined,
  ...
  if (session.phase === 'ready' && viewer) return <div className="play-reading">
    <CharacterSettings key={viewer.id} settings={characterSettings} characterId={viewer.id} onBack={onUndo} footer={<footer className="play-reading__start"><p>설정서와 진실 카드 2장을 읽고 시작하세요.</p><Button variant="primary" className="play-primary" onClick={() => act({ type: 'start' })}>{viewer.name}으로 시작하기 <Icon name="arrowRight" /></Button></footer>} />
  </div>
  ...
  return <section className="play" data-view={view} data-phase={session.phase} data-choosing={!viewer || undefined} style={{ '--actor-color': viewer?.color ?? actor.color } as CSSProperties}>
    <header className="play-toolbar">
      ...
      <nav className="play-tabs" aria-label="플레이 화면">
        {[['table', '테이블'], ['character', '내 인물'], ['record', `공개 기록 ${session.publicCards.length}`]].map(([id, title]) => <Button size="compact" variant="ghost" key={id} aria-label={id === 'table' ? '게임 테이블' : title} aria-pressed={view === id} onClick={() => update({ view: id === 'table' ? null : id, card: null, evidence: null })}>{title}</Button>)}
      </nav>
      <div className="play-history-tools"><Button size="compact" variant="ghost" onClick={onUndo} disabled={cursor === 0} aria-label="이전 선택으로 되돌리기"><Icon name="undo" /></Button><Button size="compact" variant="ghost" onClick={onRedo} disabled={cursor === lastStep} aria-label="다음 선택 복원"><Icon name="redo" /></Button><Button size="compact" variant="ghost" onClick={() => update({ history: '1' })}><Icon name="history" size={16} />기록 · {cursor}</Button></div>
    </header>
    <div className="play-phase-control">
      <Button size="compact" popoverTarget="play-overview" aria-label="진행 현황 열기"><span>{session.round} / 3 R</span><strong>{viewer ? phaseTitle(session) : '플레이테스트'}</strong><small>{turnLabel}</small><Icon name="chevronDown" size={16} /></Button>
    </div>
    <div id="play-overview" className="play-overview ui-panel" popover="auto" role="dialog" aria-label="진행 현황" onToggle={event => setOverviewOpen(event.currentTarget.matches(':popover-open'))}>
      <header><strong>진행 현황</strong><Button size="compact" popoverTarget="play-overview" popoverTargetAction="hide" aria-label="진행 현황 닫기"><Icon name="close" /></Button></header>
      <div className="play-progress" aria-label="라운드 진행">
        {phases.map((phase, index) => <div key={phase.id} className={index === phaseIndex ? 'is-current' : index < phaseIndex ? 'is-done' : ''} aria-current={index === phaseIndex ? 'step' : undefined}><span>{index < phaseIndex ? <Icon name="check" size={14} /> : `0${index + 1}`}</span>{phase.label}</div>)}
      </div>
      ...
      <Button variant="ghost" className="play-text-button" onClick={event => { event.currentTarget.closest<HTMLElement>('[popover]')?.hidePopover(); newGame() }}>다른 인물로 새 테스트</Button>
      </>}
      <footer>카드에 올려 읽기 · 드래그로 정렬 · 클릭/Space로 확대</footer>
    </div>
    {viewer && <div className="play-hud">
      <Button size="compact" className="play-my-hand" popoverTarget="play-overview" aria-label="내 카드 수집 현황 열기"><span className="play-avatar" style={{ '--seat-color': viewer.color } as CSSProperties}>{viewer.name.slice(0, 1)}</span><strong>{viewer.name}</strong><span>손패 <b>{myHand.length}</b></span><small>제출 {mySubmitted.length}</small><Icon name="chevronUp" size={16} /></Button>
      <div className="play-auto-tools"><span role="status">{autoLabel}</span>{session.phase !== 'complete' && <Button size="compact" onClick={onToggleAuto} disabled={cursor < lastStep} aria-label={autoRunning ? '자동 진행 멈추기' : '자동 진행 재개'}><Icon name={autoRunning ? 'pause' : 'play'} size={16} />{autoRunning ? '멈춤' : '자동 진행'}</Button>}</div>
    </div>}
    <div className="play-surface">
      <div className="play-table-mark" aria-hidden="true"><Icon name="crown" size="1em" /></div>
      {view === 'record' ? <PublicRecord session={session} onRead={readCard} /> : view === 'character' && viewer ? <CharacterSheet actor={viewer} session={session} onRead={readCard} /> : <>
        ...
        {!viewer ? <div className="play-intro">
          <span className="eyebrow">인물 선택</span><h3>플레이할 인물을 선택하세요.</h3>
          <div className="play-person-choices play-perspective-choices">{scenario.characters.map(character => <Button variant="choice" key={character.id} onClick={() => act({ type: 'play-as', playerId: character.id })} style={{ '--seat-color': character.color } as CSSProperties}><span>{character.name.slice(0, 1)}</span><strong>{character.name}</strong><small>{character.title}</small><em>{session.phase === 'ready' ? '설정서 먼저 읽기' : '이 인물로 이어서 플레이'} <Icon name="arrowRight" size={16} /></em></Button>)}</div>
          <small>{session.phase === 'ready' ? '인물을 선택하면 설정서를 읽습니다.' : '선택한 인물로 현재 게임을 이어갑니다.'}<br />다른 인물은 추리 없이 무작위로 행동합니다.</small>
          ...
          {session.phase === 'inspection' && currentInspection && <div className="play-inspection-result"><div className="play-feature-card"><CardView card={cards.get(currentInspection.cardId)!} onClick={() => readCard(currentInspection.cardId)} /></div><div><span className="play-private-label"><Icon name="lock" size={14} /> 로웬만 확인</span><h4>검시 결과를 확인하세요.</h4><p>검시 항목은 공개됩니다. 결과는 로웬만 읽고,<br />이번 재판이 시작되면 모두에게 공개됩니다.</p><Button variant="primary" className="play-primary" onClick={() => act({ type: 'finish-inspection' })}>소문 조사하기 <Icon name="arrowRight" /></Button></div></div>}
          {session.phase === 'discussion' && <div className="play-discussion"><div className="play-discussion-symbol"><Icon name="moon" size="1em" /></div><h4>재판을 준비하세요.</h4><p>단서를 비교하고 제출할 카드 1장을 정하세요.</p><div className="play-inspection-sealed"><Icon name="lock" size={16} /> {cards.get(currentInspection?.cardId ?? '')?.title} · 재판 개정 시 공개</div><Button variant="primary" className="play-primary" onClick={() => act({ type: 'end-discussion' })}>제{session.round}재판 시작하기 <Icon name="arrowRight" /></Button></div>}
          {session.phase === 'court' && (session.courtTurn ? <div className="play-question"><div className="play-feature-card"><CardView card={cards.get(session.courtTurn.cardId)!} onClick={() => readCard(session.courtTurn!.cardId)} /></div><div><span className="eyebrow">증거 제출 완료</span><h4>{actor.name} → {player(session.courtTurn.targetId)?.name}</h4><p>{session.round === 2 ? '제출한 증거로 혐의를 주장하고 질문하세요.' : '이 증거와 관련된 질문을 한 번 하세요.'}<br />질문받은 인물은 자유롭게 답합니다.</p><Button variant="primary" className="play-primary" onClick={() => act({ type: 'end-question' })}>질문·답변 마치기 <Icon name="arrowRight" /></Button></div></div> : pendingEvidence ? <div className="play-submit"><div className="play-feature-card"><CardView card={pendingEvidence} onClick={() => readCard(pendingEvidence.id)} /></div><div className="play-submit-panel ui-panel"><h4>{session.round === 3 ? '마지막 카드를 제출하세요.' : session.round === 2 ? '혐의를 제기할 상대를 선택하세요.' : '질문할 상대를 선택하세요.'}</h4><p>제출한 카드는 손패에서 빠지고 모두에게 공개됩니다.</p>{session.round === 3 ? <Button variant="primary" className="play-primary" onClick={() => act({ type: 'submit-evidence', cardId: pendingEvidence.id, targetId: '' })}>제출하기 <Icon name="arrowRight" /></Button> : <CharacterChoices actorId={actor.id} onChoose={targetId => act({ type: 'submit-evidence', cardId: pendingEvidence.id, targetId })} />}<Button variant="ghost" className="play-text-button" onClick={() => update({ evidence: null })}>다른 증거 선택</Button></div></div> : null)}
          ...
          {session.phase === 'accusation' && <div className="play-nominate"><span className="play-private-label">비공개 지목 · {Object.keys(session.accusations).length} / 5 제출</span><h4>범인으로 의심하는 인물을 지목하세요.</h4><p>자신과 로웬은 지목할 수 없습니다. 지목은 동시에 공개합니다.<br />최종 기소는 변론 뒤 로웬이 결정합니다.</p><CharacterChoices actorId={actor.id} excludeRowen onChoose={targetId => act({ type: 'accuse', targetId })} /></div>}
          {session.phase === 'defense' && <div className="play-defense"><Nominations session={session} /><h4>{actor.name}의 최후변론</h4><p>{actor.id === 'rowen' ? '공개된 증거와 변론을 바탕으로 수사 결론을 정리하세요.' : <>“{player(session.accusations[actor.id])?.name}이 범인이라고 생각하는 이유는…”<br />“그리고 내가 범인이 아닌 이유는…”</>}</p><span className="play-subtle">공개 증거를 근거로 변론합니다. 이 단계에는 질문·반박이 없습니다.</span><Button variant="primary" className="play-primary" onClick={() => act({ type: 'end-defense' })}>변론 마치기 <Icon name="arrowRight" /></Button></div>}
          {session.phase === 'indictment' && <div className="play-nominate"><Nominations session={session} /><h4>기소할 인물 1명을 선택하세요.</h4><p>공개 증거·손패·최후 변론을 근거로 결정하세요. 앞선 지목과 달라도 됩니다.</p><CharacterChoices actorId="rowen" excludeRowen onChoose={targetId => act({ type: 'indict', targetId })} confirm /></div>}
    ...
    <CardReader card={canvasReadingId ? undefined : reading} onClose={() => update({ card: null })}>
      {reading && session.choice?.cardIds.includes(reading.id) && ownTurn ? <Button variant="primary" className="play-primary" onClick={() => take(reading.id)}>이 카드 가져오기 <Icon name="arrowRight" /></Button> : reading && session.phase === 'court' && !session.courtTurn && evidence.some(card => card.id === reading.id) && ownTurn ? <Button variant="primary" className="play-primary" onClick={() => submit(reading.id)}>이 증거를 재판에 제출 <Icon name="arrowRight" /></Button> : null}
    </CardReader>
  </section>
}
...
function TruthExchange({ session, actor, act }: { session: PlaySession; actor: Character; act: (action: PlayAction) => void }) {
  ...
  return <div className="play-nominate play-truth-exchange">
    ...
    <h4>교환할 진실 1장을 선택하세요.</h4>
    <p>상대와 합의해 1장씩 교환합니다. 내용은 말할 수 있지만 앞면은 보여 줄 수 없습니다.{actor.id === 'rowen' && <><br />로웬은 원래 자신의 진실만 내놓을 수 있습니다.</>}</p>
    <div className="play-record-cards">{offered.map(id => <div key={id}><CardView card={cards.get(id)!} /><Button variant="primary" className="play-primary" aria-pressed={offeredId === id} onClick={() => setOfferedId(id)}>{offeredId === id ? '내놓을 진실로 선택됨' : '이 진실을 내놓기'}</Button></div>)}</div>
    {offeredId && <CharacterChoices actorId={actor.id} onChoose={id => { setTargetId(id); setRequestedId('') }} />}
    {targetId && <div className="play-person-choices">{targetTruths.map((id, index) => <Button variant="choice" key={id} aria-pressed={requestedId === id} onClick={() => setRequestedId(id)}><span>?</span><strong>묻어야 하는 진실 {index + 1}</strong><small>교환 뒤에만 내용을 확인합니다.</small></Button>)}</div>}
    {offeredId && targetId && requestedId && <Button variant="primary" className="play-primary" onClick={() => act({ type: 'trade-truth', offeredId, targetId, requestedId })}>{player(targetId).name}과 1장 ↔ 1장 교환 <Icon name="arrowRight" /></Button>}
    <Button variant="ghost" className="play-text-button" onClick={() => act({ type: 'skip-truth-trade' })}>이번에는 교환하지 않기</Button>
  </div>
}
...
function TruthChoice({ session, actor, act }: { session: PlaySession; actor: Character; act: (action: PlayAction) => void }) {
  ...
  return <div className="play-nominate play-truth-choice">
    ...
    <h4>공개할 진실 1장을 선택하세요.</h4>
    <p>나머지 1장은 묻습니다. 선택은 기소 뒤 공개합니다.<br />기소된 사람의 손에 있는 진실과 자기 손에 남은 자신의 진실은 모두 공개됩니다.</p>
    <div className="play-record-cards">{held.map(id => <div key={id}><CardView card={cards.get(id)!} /><Button variant="primary" className="play-primary" onClick={() => act({ type: 'choose-truth', revealId: id })}>이 진실 공개하기 <Icon name="arrowRight" /></Button></div>)}</div>
  </div>
}
...
function TruthEnding({ session, onRead, onRecord, newGame }: { session: PlaySession; onRead: (id: string) => void; onRecord: () => void; newGame: () => void }) {
  ...
  return <div className="play-complete play-truth-ending"><span>⚖</span><h4>{player(session.indictment ?? '')?.name} 기소</h4><p>기소에 따른 진실 공개 결과입니다.</p>
    <section className="play-truth-group play-truth-group--revealed"><h5><span>공개된 진실</span><b>{revealed.length}</b></h5><p>공개된 진실의 결과를 재판 기록과 함께 확인하세요.</p><div className="play-record-cards">{revealed.map(id => <CardView key={id} card={cards.get(id)!} onClick={() => onRead(id)} />)}</div></section>
    <section className="play-ending-consequences"><h5>공개된 진실의 결과</h5>{characterSettings.flatMap(setting => setting.finalActions).filter(truth => revealed.includes(truth.id)).map(truth => <div key={truth.id}><strong>{truth.title}</strong><p>{truth.omen}</p></div>)}</section>
    <section className="play-truth-group play-truth-group--buried"><h5><span>묻힌 진실</span><b>{buried.length}</b></h5><p>묻힌 카드의 내용은 공개하지 않습니다.</p><div className="play-record-cards">{buried.map(id => <CardView key={id} card={cards.get(id)!} faceDown backTitle="묻힌 진실" backLabel="내용이 공개되지 않은 진실" />)}</div></section>
    <small>기소된 사람이 가진 진실과 자기 손에 남은 자기 진실은 모두 밝혀졌습니다.</small><Button variant="primary" className="play-primary" onClick={onRecord}>공개 기록 보기 <Icon name="arrowRight" /></Button><Button variant="ghost" className="play-text-button" onClick={newGame}>기존 기록을 남기고 새 플레이</Button>
  </div>
}
...
function CharacterChoices({ actorId, onChoose, confirm = false, excludeRowen = false }: { actorId: string; onChoose: (id: string) => void; confirm?: boolean; excludeRowen?: boolean }) {
  ...
  return <div className="play-person-choices">{scenario.characters.filter(character => character.id !== actorId && (!excludeRowen || character.id !== 'rowen')).map(character => <Button variant="choice" key={character.id} onClick={() => confirm ? setSelected(character.id) : onChoose(character.id)} aria-pressed={confirm ? selected === character.id : undefined} style={{ '--seat-color': character.color } as CSSProperties}><span>{character.name.slice(0, 1)}</span><strong>{character.name}</strong><small>{character.title}</small></Button>)}{confirm && selected && <Button variant="primary" className="play-primary play-indict-confirm" onClick={() => onChoose(selected)}>{player(selected).name} 기소 확정 <Icon name="arrowRight" /></Button>}</div>
}

function Nominations({ session }: { session: PlaySession }) {
  return <div className="play-nominations">{scenario.characters.filter(character => character.id !== 'rowen').map(character => <div key={character.id}><small>{character.name}</small><Icon name="arrowRight" size={16} /><strong>{player(session.accusations[character.id])?.name ?? '미제출'}</strong></div>)}</div>
}

function PublicRecord({ session, onRead }: { session: PlaySession; onRead: (id: string) => void }) {
  return <div className="play-record"><header><span className="eyebrow">공개 기록</span><h3>재판에 제출된 기록</h3><p>제출한 카드와 공개된 검시 결과를 확인하세요.</p></header>
    ...
    {session.phase === 'complete' && <section><h4>진실 공개 결과</h4><div className="play-record-cards">{Object.entries(session.truthOutcomes).filter(([, outcome]) => outcome === 'revealed').map(([id]) => <div key={id}><small>공개</small><CardView card={cards.get(id)!} onClick={() => onRead(id)} /></div>)}</div><p>{Object.values(session.truthOutcomes).filter(outcome => outcome === 'buried').length}개의 진실은 끝내 묻혔습니다.</p></section>}
  </div>
}
...
function CharacterSheet({ actor, session, onRead }: { actor: Character; session: PlaySession; onRead: (id: string) => void }) {
  ...
  return <div className="play-character"><header><span className="eyebrow">{actor.title} · PRIVATE</span><h3>{actor.name}</h3><p>{setting?.objective ?? actor.desire}</p></header><div className="play-character-memories">{session.hands[actor.id].filter(id => cards.get(id)?.kind === 'memory').map(id => <CardView key={id} card={cards.get(id)!} onClick={() => onRead(id)} />)}</div>
    {setting && <section><h4>당신의 신념</h4><p>{setting.belief}</p><ul>{setting.goals.map(goal => <li key={goal}>{goal}</li>)}</ul><p>범인 추리와 개인 목표는 따로 판정합니다.</p></section>}
    {setting && <section><h4>내 진실의 공개 결과</h4><p>자신의 진실 2장이 모두 공개되면 ‘발각’됩니다. 결과는 개인 설정서를 따릅니다.</p>{setting.finalActions.map(truth => <div key={truth.id}><strong>{truth.title}</strong><p>{truth.intent}</p></div>)}</section>}
...
function stageHint(session: PlaySession) {
  if (session.choice) return `${groups.find(group => group.id === session.choice!.deckId)?.backTitle ?? '선택한 덱'}에서 확인한 카드입니다. 1장을 선택하세요.`
  return ({ ready: '묻어야 할 진실 두 장을 받고 조사와 재판을 시작합니다.', inspection: session.round === 1 ? '첫 검시 항목은 시신 전반입니다.' : '검시할 항목 1개를 선택하세요.', rumor: '소문 덱을 열고 3장 중 1장을 가져가세요.', testimony: '아직 만나지 않은 NPC 1명을 선택하세요. 이번 라운드에 다른 사람이 고른 NPC는 선택할 수 없습니다.', investigation: '장소를 골라 2장 중 1장을 가져가세요. 나머지는 덱 아래에 넣습니다. 1장만 남으면 그대로 가져갑니다.', discussion: '모은 단서를 토의하고 제출할 카드를 정하세요.', court: `제${session.round}재판 · 각자 한 장씩 공개 증거를 제출합니다.`, truth_exchange: '상대와 합의해 진실을 1장씩 뒷면으로 교환하세요.', truth_choice: '공개할 진실 1장을 선택하세요. 선택은 기소 뒤 공개합니다.', accusation: '로웬을 제외한 다섯 사람 중 진범이라고 생각하는 사람을 지목합니다.', defense: '범인 지목이 공개되었습니다. 로웬은 마지막에 수사 결론을 정리합니다.', indictment: '로웬은 공개 증거와 가진 모든 카드로 한 사람을 기소합니다.', complete: '진실 공개 결과와 결말을 확인하세요.' } as Record<PlayPhase, string>)[session.phase]
}

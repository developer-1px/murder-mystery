...
export function TimelineBoard({ scenario, document }: { scenario: Scenario; document: TimelineDocument }) {
  ...
  const columns = [...scenario.characters.map((character) => ({ id: character.id, label: character.name })), { id: 'common', label: '공통 · 루시엔 포함' }]
  ...
  return <section className="timeline-board">
    ...
    <p className="timeline-board__guide">각 카드는 사건의 주요 관점으로 분류합니다. 루시엔은 아드리안의 NPC이며 공통 열에 표시합니다. 이 표의 사건 관련 인물과 인물별 전용 카드 배정은 다를 수 있습니다. 전용 장수는 카드 구성 화면을 기준으로 합니다.</p>

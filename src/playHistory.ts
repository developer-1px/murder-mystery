...
export const playStorageKey = (scenarioId: string) => `murder-mystery:play:${scenarioId}:v1`
...
export function createPlayHistory(assets: PlayAssets): PlayHistory {
  ...
  return { version: 1, branches: [{ id: session.id, name: '첫 번째 재판', snapshots: [session] }], last: { branchId: session.id, step: 0 } }
}
...
export function readPlayHistory(assets: PlayAssets): { history: PlayHistory; error?: string; warning?: string } {
  ...
  try {
    ...
    const phases = new Set(['ready', 'inspection', 'rumor', 'testimony', 'investigation', 'discussion', 'court', 'accusation', 'defense', 'indictment', 'complete'])
    ...
    if (history.version !== 1 || !Array.isArray(history.branches) || !history.branches.length) throw new Error('format')
    ...
    for (const branch of history.branches) {
      ...
      for (const session of branch.snapshots) {
        ...
        if (!session || !phases.has(session.phase) || !characterIds.has(session.actorId) || !Number.isInteger(session.round) || session.round < 1 || session.round > 3 || !Array.isArray(session.log) || !Array.isArray(session.publicCards) || !Array.isArray(session.inspections) || !session.decks || !session.hands || !session.visitedNpcs || !session.accusations || !session.locationChoices) throw new Error('session')

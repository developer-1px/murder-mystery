...
export const playStorageKey = (scenarioId: string) => `murder-mystery:play:${scenarioId}:v2`
...
export function createPlayHistory(assets: PlayAssets): PlayHistory {
  ...
  return { version: 2, branches: [{ id: session.id, name: '첫 번째 재판', snapshots: [session] }], last: { branchId: session.id, step: 0 } }
}
...
export function readPlayHistory(assets: PlayAssets): { history: PlayHistory; error?: string; warning?: string } {
  ...
  try {
    ...
    if (history.version !== 2 || !Array.isArray(history.branches) || !history.branches.length) throw new Error('format')

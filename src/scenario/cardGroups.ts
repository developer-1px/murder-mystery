...
export function getCardGroups(scenario: Scenario, npcGroups: NpcGroupsDocument, memoryStages: MemoryStagesDocument): CardGroup[] {
  ...
  return [
    ...
    ...npcGroups.npcs.map((npc) => ({
      ...
      description: `${scenario.characters.find((character) => character.id === npc.pairedCharacterId)?.name ?? (scenario.meta.victim?.id === npc.pairedCharacterId ? scenario.meta.victim.name : npc.pairedCharacterId)}와 연결된 탐문 NPC. 두 장을 읽고 한 장을 획득하며 나머지는 덱에 남깁니다.`,
    ...
    ...scenario.locations.map((location) => ({
      ...
      description: `${location.description} 구역마다 8장입니다. 자기 차례에 장소를 골라 2장 중 1장을 가져갑니다. 남은 1장은 덱 아래로 돌립니다. 1장만 남으면 그 카드를 가져갑니다.`,
